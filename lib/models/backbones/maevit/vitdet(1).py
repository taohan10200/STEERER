...
class ViT(nn.Module):
    ...
    def forward_features(self, x, mask=None):
        ...
        if self.use_cls_token:  # for reid baseline exp only
            output = {}
            output['cls_feats'] = x[:, 0]
            output['patch_feats'] = x[:, 1:].permute(0, 2, 1).reshape(B, -1, Hp, Wp)
            return output
...
def vit_base_patch16(pretrained=False, load_pos_embed=True, pretrain_path=None, pos_embed_interp=False, **kwargs):
    default = dict(
    ...
    if pretrained:
        ...
        if pretrain_path:
            checkpoint = torch.load(pretrain_path)['model']
            print("load pretrain from {}".format(pretrain_path))
        elif pretrained == 'supervised-80ecf9dd':
        ...
        load_checkpoint(model, checkpoint, load_pos_embed, strict=False, pos_embed_interp=pos_embed_interp, logger=dummy_logger)
        del checkpoint
...
def vit_large_patch16(pretrained=False, load_pos_embed=True, pretrain_path=None, pos_embed_interp=False, **kwargs):
    default = dict(
    ...
    if pretrained:
        assert pretrain_path != None, "pretrain path not None"
        checkpoint = torch.load(pretrain_path)['model']
        print('pretrain load from {}'.format(pretrain_path))
        # load while interpolates position embedding
        load_checkpoint(model, checkpoint, load_pos_embed, strict=False, pos_embed_interp=pos_embed_interp, logger=dummy_logger)
        del checkpoint
...
def load_checkpoint(model, state_dict, load_pos_embed, strict=False, pos_embed_interp=True, logger=None):
    """
    Args:
    ...
    if 'pos_embed' in state_dict:
        if load_pos_embed:
            if not pos_embed_interp:
                pass
            elif model.use_cls_token:
